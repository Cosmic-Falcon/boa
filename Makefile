...
.PHONY : all clean
all: $(OBJS)
	$(AR) $(ARFLAGS) $(OUT_DIR)/lib$(OUT).a $(OBJS)
	cp $(SRC_DIR)/$(OUT).h $(OUT_DIR)/$(OUT).h

shared: CFLAGS += -fpic
shared: $(OBJS)
	$(CC) -shared $(OBJS) $(INCLUDE_DIRECTORIES) $(LIBRARY_DIRECTORIES) $(LIBRARIES) -o $(OUT_DIR)/lib$(OUT).so # Not sure if linking libraries works when compiling shared libraries
	cp $(SRC_DIR)/$(OUT).h $(OUT_DIR)/$(OUT).h
