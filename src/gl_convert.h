...
template<typename T> requires requires (T t) {
	{t.length()} -> std::size_t;
} std::size_t get_num_elements(T t) {
	return t.length();
}
...
GLData gen_gl_data(const Vertices vertices, std::vector<AttributeContainer> attribute) {
	GLData data;
	data.num_verts = vertices.size();
	data.num_elements = (vertices.size() - 2) * 3;

	data.num_attributes = 3 + get_num_elements(attribute[0]);
